 ...
 * handling, JSON, cursor invariants -- is plain Kotlin with no platform in it and no network in it,
 * so all of it is unit-testable against a fake that returns canned responses. Everything below it
 * is one HTTP call. That split is the reason this module has real tests for the parts that are easy
 * to get wrong and no tests at all for the part that is not.
 *
