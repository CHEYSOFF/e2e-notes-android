...
internal object Base64Codec {
     ...
     *
     * Used for exactly one thing: OkHttp's `CertificatePinner` -- reached through Ktor's OkHttp
     * engine, in `okHttpPin` -- takes its pins as
     ...
     * It lives here, in common code, rather than beside its one caller so that an Apple engine that
     * needs the same digest in the same form does not grow a second copy of it.
     */
