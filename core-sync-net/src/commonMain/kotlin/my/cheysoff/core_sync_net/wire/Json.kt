 ...
 *    and says so.
 *  - **`kotlinx.serialization`** would mean applying its compiler plugin and generating serializers
 *    for a handful of flat objects. It is now a smaller step than it was -- this module applies the
 *    Kotlin multiplatform plugin, so the argument that no module here applies a Kotlin Gradle plugin
 *    at all no longer holds -- but the reason to keep this file is no longer the build: the wire is
 *    strict in ways a generated serializer is not. A duplicate object key is refused, an unknown
 *    field is ignored on the way in and would be a `400` on the way out, and integers never pass
 *    through a `Double`. Those are decisions this file makes visibly, in the same place the field
 *    names live.
 *  - **Platform JSON** does not exist; there is no JSON in the Java SE 11 standard library and none
...
internal class JsonWriter {
    ...
    /** `encodeToByteArray` rather than `toByteArray(Charsets.UTF_8)`: the latter is JVM-only. */
    fun toBytes(): ByteArray = out.toString().encodeToByteArray()
