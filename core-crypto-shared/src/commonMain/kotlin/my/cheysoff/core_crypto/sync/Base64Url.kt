 ...
 * on both devices.
 *
 * In `commonMain` rather than `jvmCommonMain` with the rest of this module, and that placement is
 * load-bearing: :core-sync-net's transport is common code and encodes every public key, signature
 * and account handle it sends with THIS encoder. A copy on the transport side would be a second
 * implementation of one primitive, which is the failure this project has already shipped once (two
 * HKDFs). Nothing here touches the JVM, so nothing had to change to move it.
 */
object Base64Url {
