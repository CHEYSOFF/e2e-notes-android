...
class SyncEngine(
    ...
    private suspend fun pull(): Phase {
        ...
        while (true) {
            val page = try {
                ...
                return finishPull(committable, startCursor, stats, stop(failure), rebaselining)
            }
                ...
                when (incoming) {
                    is IncomingRecord.Faulted -> when (incoming.fault) {
                        RecordFault.UNREADABLE -> {
                            ...
                            if (stats.unreadable > UNREADABLE_RECORD_LIMIT) {
                                return finishPull(
                                    ...
                                    halt(HaltReason.RECORDS_UNREADABLE), rebaselining,
                                )
                            }
                        }
                        ...
                        RecordFault.MISLABELLED -> return finishPull(
                            committable, startCursor, stats,
                            halt(HaltReason.RECORD_MISLABELLED), rebaselining,
                        )

                        RecordFault.UNSUPPORTED_PAYLOAD_VERSION -> return finishPull(
                            committable, startCursor, stats,
                            halt(HaltReason.UNSUPPORTED_PAYLOAD_VERSION), rebaselining,
                        )
                    ...
                    is IncomingRecord.Opened -> {
                        ...
                        stats += applied.stats
                        applied.stop?.let {
                            return finishPull(committable, startCursor, stats, it, rebaselining)
                        }
                        if (!frozen) committable = incoming.seq
                    }
                }
        ...
        return finishPull(committable, startCursor, stats, null, rebaselining)
    }

    /**
     * Persists the cursor if it moved, records a completed re-baseline, and packages the phase's
     * result.
     *
     * @param rebaselining whether this pull started from 0 to recover records a lower format
     *   generation skipped. Threaded in as a parameter rather than read from a field: the engine
     *   runs one pass at a time under [pass], but a field here would be pass state that outlives
    ...
    private suspend fun finishPull(
        ...
        // Only on a pull that reached its end with nothing stopping it. A re-baseline cut short by
        // a dropped connection has to run again in full; recording the generation here would tell
        // the device it had caught up while the records the interruption cost it are still behind
        // its cursor.
        if (rebaselining && stop == null) store.saveDataVersion(DATA_VERSION)
