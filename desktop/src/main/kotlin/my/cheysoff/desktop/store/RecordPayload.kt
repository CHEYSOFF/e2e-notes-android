 ...
 * it.
 *
 * The phone's `RecordCodec` does not exist yet — §5.1 is a plan, not code — so this is the first
 * implementation of that format, and it is written to the plan rather than to what is convenient
 * here so that the phone can adopt it unchanged. Two places where the plan needed a decision made
 * are marked below; both are the kind of thing that must be settled identically on both ends or the
 * two devices disagree silently.
 *
 * ```
...
object RecordPayloadCodec {
    ...
    fun encode(payload: RecordPayload): ByteArray {
        ...
        val obj = buildJsonObject {
            ...
            // source of truth. Whichever the phone's codec ends up preferring, it will not be able
            // to write a record this one silently misreads.
    ...
    fun decode(bytes: ByteArray): PayloadResult = try {
        ...
        PayloadResult.Malformed(e.message ?: e::class.java.simpleName)
    }
    ...
    private fun isDeleted(payload: RecordPayload): Boolean =
        payload.field(PayloadFields.IS_DELETED) == my.cheysoff.core_domain.sync.SyncValues.TRUE
}
